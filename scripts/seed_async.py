...
CINEMAS = [{'name': 'Vanitha Vineetha', 'screens': 2}]
CITY = 'Kochi'
...
async def main():
    ...
    async with S() as s:
        ...
        u = (await s.execute(select(User).where(User.email=='demo@kochi2.local'))).scalar_one_or_none()
        if not u:
            u = User(id=uuid.uuid4(), email='demo@kochi2.local', password_hash=pwd.hash('demo1234'), role='PARTNER')
